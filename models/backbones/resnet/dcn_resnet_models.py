...
from extensions.dcn import ModulatedDeformConv, ModulatedDeformRoIPoolingPack, DeformConv
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.bn3 = ModuleHelper.BatchNorm2d(
            bn_type=bn_type)(planes * self.expansion)
        self.relu = nn.ReLU(inplace=False)
        self.relu_in = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
        ...
        def _inner_forward(x):
            ...
            if self.with_dcn:
                if self.use_modulated_dcn:
                    ...
                    offset1, offset2, mask_raw = torch.chunk(
                        offset_mask, 3, dim=1)
                    ...
                    bias_w = torch.cuda.FloatTensor(
                        [[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]]) * (dilation - 1)
                    bias_h = bias_w.permute(1, 0)
                    bias_w.requires_grad = False
                    bias_h.requires_grad = False
                    offset += torch.cat([bias_h.reshape(-1),
                                        bias_w.reshape(-1)]).view(1, -1, 1, 1)
