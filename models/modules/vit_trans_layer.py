...
from models.modules.norm import trunc_normal_
import math
...
class Attention_Decoder(nn.Module):
    ...
    def forward(self, q, x):
        ...
        q = self.fc_q(q).reshape(B, self.num_heads,
                                 n_class, C // self.num_heads)
        kv = self.fc_kv(x).reshape(B, N, 2, self.num_heads, C //
                                   self.num_heads).permute(2, 0, 3, 1, 4)
...
class Block_Decoder(nn.Module):
    ...
    def __init__(self, dim, num_heads, feat_HxW, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        self.mlp3 = Mlp(in_features=feat_HxW, hidden_features=feat_HxW *
                        3, act_layer=act_layer, drop=drop)
...
class VisionTransformer(nn.Module):
    ...
    def resize_pos_embed(self, x, pos_embed, h, w):
        ...
        cls_pos_embed, feat_pos_embed = pos_embed[:,
                                                  0:1, :], pos_embed[:, 1:, :]
