...
class ASPPModuleV2(nn.Module):
    ...
    def __init__(self, features, inner_features=512, out_features=512, dilations=(12, 24, 36), bn_type=None, dropout=0.1):
        ...
        self.conv_3x3_1 = nn.Sequential(
            nn.Conv2d(features, inner_features, kernel_size=3,
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(inner_features * 4, out_features,
...
class ASPPModule(nn.Module):
    ...
    def forward(self, x):
        _, _, h, w = x.size()
        feat_gp = F.interpolate(self.conv_gp(x), size=(
...
if __name__ == "__main__":
    ...
    if int(os.environ.get('eval_os_8', 1)):
        print(
            "Complexity Evaluation Results for ASPP with input shape [2048 X 128 X 128]")
        feats = torch.randn((1, 2048, 128, 128)).cuda()
        aspp_infer = ASPPModule(2048, 256, 256, bn_type=custom_bn_type)
    else:
        print(
            "Complexity Evaluation Results for ASPP with input shape [720 X 256 X 512]")
