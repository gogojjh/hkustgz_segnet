...
class SpatialGather_Module(nn.Module):
    ...
    def forward(self, feats, probs, gt_probs=None):
        if self.use_gt and gt_probs is not None:
            gt_probs = label_to_onehot(gt_probs.squeeze(
                1).type(torch.cuda.LongTensor), probs.size(1))
            batch_size, c, h, w = gt_probs.size(0), gt_probs.size(
                1), gt_probs.size(2), gt_probs.size(3)
            gt_probs = gt_probs.view(batch_size, c, -1)
            feats = feats.view(batch_size, feats.size(1), -1)
            feats = feats.permute(0, 2, 1)  # batch x hw x c
            gt_probs = F.normalize(gt_probs, p=1, dim=2)  # batch x k x hw
            ocr_context = torch.matmul(gt_probs, feats).permute(
...
class PyramidSpatialGather_Module(nn.Module):
    ...
    def _compute_single_scale(self, feats, probs, dh, dw):
        batch_size, k, h, w = probs.size(0), probs.size(
            1), probs.size(2), probs.size(3)
        c = feats.size(1)
        ...
        if pad_h > 0 or pad_w > 0:  # padding in both left&right sides
            feats = F.pad(feats, (pad_w // 2, pad_w - pad_w //
                          2, pad_h // 2, pad_h - pad_h // 2))
            probs = F.pad(probs, (pad_w // 2, pad_w - pad_w //
                          2, pad_h // 2, pad_h - pad_h // 2))

        feats = feats.view(batch_size, c, out_h, dh, out_w,
                           dw).permute(0, 3, 5, 1, 2, 4)
        feats = feats.contiguous().view(batch_size, dh * dw, c, out_h, out_w)

        probs = probs.view(batch_size, k, out_h, dh, out_w,
                           dw).permute(0, 3, 5, 1, 2, 4)
        ...
        cc = torch.matmul(probs, feats).view(
            batch_size, -1, c)  # batch x k x c
...
class _ObjectAttentionBlock(nn.Module):
    ...
    def forward(self, x, proxy, gt_label=None):
        ...
        if self.use_gt and gt_label is not None:
            gt_label = label_to_onehot(gt_label.squeeze(1).type(
                torch.cuda.LongTensor), proxy.size(2) - 1)
        ...
        if self.use_bg:
            ...
            bg_context = bg_context.view(
                batch_size, self.key_channels, *x.size()[2:])
            bg_context = self.f_up(bg_context)
            bg_context = F.interpolate(input=bg_context, size=(
                h, w), mode='bilinear', align_corners=True)
    ...
    def forward(self, feats, proxy_feats, gt_label=None):
        if self.use_gt and gt_label is not None:
            if self.use_bg:
                ...
                context = self.object_context_block(
                    feats, proxy_feats, gt_label)
        else:
            if self.fetch_attention:
                context, sim_map = self.object_context_block(
                    feats, proxy_feats)
            else:
...
class SpatialOCR_ASP_Module(nn.Module):
    def __init__(self, features, hidden_features=256, out_features=512, dilations=(12, 24, 36), num_classes=19,
        ...
        from models.modules.spatial_ocr_block import SpatialOCR_Context
        self.context = nn.Sequential(
            nn.Conv2d(features, hidden_features, kernel_size=3,
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(features, hidden_features, kernel_size=1,
        ...
        self.conv5 = nn.Sequential(
            ...
            ModuleHelper.BNReLU(hidden_features, bn_type=bn_type), )
        self.conv_bn_dropout = nn.Sequential(
            nn.Conv2d(hidden_features * 5, out_features,
    ...
    def _cat_each(self, feat1, feat2, feat3, feat4, feat5):
        ...
        for i in range(len(feat1)):
            z.append(
                torch.cat((feat1[i], feat2[i], feat3[i], feat4[i], feat5[i]), 1))
        return z
...
if __name__ == "__main__":
    ...
    print("Average Parameters : {}".format(count_parameters(
        ocp_distr_infer) + count_parameters(conv_3x3)))
