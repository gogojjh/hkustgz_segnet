...
class PSPModule(nn.Module):
    ...
    def __init__(self, features, out_features=512, sizes=(1, 2, 3, 6), bn_type=None):
        ...
        self.stages = nn.ModuleList(
            [self._make_stage(features, out_features, size, bn_type) for size in sizes])
...
if __name__ == "__main__":
    ...
    if int(os.environ.get('eval_os_8', 1)):
        print(
            "Complexity Evaluation Results for PPM with input shape [2048 X 128 X 128]")
        feats = torch.randn((1, 2048, 128, 128)).cuda()
        psp_infer = PSPModule(2048, bn_type=custom_bn_type)
    else:
        print(
            "Complexity Evaluation Results for PPM with input shape [720 X 256 X 512]")
