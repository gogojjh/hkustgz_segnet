...
from models.modules.basic import SeparableConv2d
...
class Attention(nn.Module):
    ...
    def forward(self, low_feature, h_feature, H, W):
        ...
        q = self.q(h_feature).reshape(B, N, self.num_heads,
        ...
        if self.sr_ratio > 1:
            ...
            kv = self.kv(low_feature).reshape(
...
class SubPixelConv(nn.Module):
    def __init__(self, img_size=224, patch_size=2, in_chans=768, embed_dim=768):
        ...
        self.upsample = nn.Upsample(
            scale_factor=self.patch_size[0], align_corners=False, mode='bilinear')
        self.upsample_proj = nn.Conv2d(
            in_chans, embed_dim, kernel_size=3, stride=1, padding=1, bias=True)
...
class AlignedModule(nn.Module):
    ...
    def __init__(self, inplane, outplane, num_heads, mlp_ratio, sr_ratio):
        ...
        self.flow_make = nn.Conv2d(
            outplane * 2, 2, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        h_feature = F.upsample(h_feature, size=size,
