...
def make_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    ...
    if quality == 'coarse':
        ...
        logging.info('{} coarse cities: '.format(
        ...
        for mode in modes:
            if mode == 'test':
                ...
                logging.info('{} fine cities: '.format(
    ...
    logging.info('Cityscapes-{}: {} images'.format(mode,
...
class CityScapes(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.eval_mode:  # eval_mode: image transforms
...
class CityScapesUniform(data.Dataset):
    ...
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        self.imgs, self.aug_imgs = make_dataset(
            quality, mode, self.maxSkip, cv_split=self.cv_split)
        ...
        if self.coarse_boost_classes is not None:
            ...
            for class_id in self.coarse_boost_classes:
                self.centroids[class_id].extend(
                    self.coarse_centroids[class_id])
