...
def collate(batch, trans_dict):
    ...
    if trans_dict['size_mode'] == 'diverse_size':
        ...
        target_widths, target_heights = [
            target_width] * len(batch), [target_height] * len(batch)
    ...
    if 'fit_stride' in trans_dict:
        ...
        for i in range(len(batch)):
            ...
            pad_w = 0 if (target_width % stride == 0) else stride - \
                (target_width % stride)  # right
            pad_h = 0 if (target_height % stride == 0) else stride - \
                (target_height % stride)  # down
    ...
    for i in range(len(batch)):
        ...
        if trans_dict['align_method'] in ['only_scale', 'scale_and_pad']:
            ...
            if 'labelmap' in data_keys:
                labelmap = batch[i]['labelmap'].data.unsqueeze(
                    0).unsqueeze(0).float()
                labelmap = F.interpolate(
                    labelmap, scaled_size_hw, mode='nearest').long().squeeze(0).squeeze(0)
                batch[i]['labelmap'] = DataContainer(labelmap, stack=True)

            if 'maskmap' in data_keys:
                maskmap = batch[i]['maskmap'].data.unsqueeze(
                    0).unsqueeze(0).float()
                maskmap = F.interpolate(
                    maskmap, scaled_size_hw, mode='nearest').long().squeeze(0).squeeze(0)
        ...
        if pad_width > 0 or pad_height > 0:
            ...
            if 'labelmap' in data_keys:
                batch[i]['labelmap'] = DataContainer(F.pad(
                    batch[i]['labelmap'].data, pad=pad, value=-1), stack=batch[i]['labelmap'].stack)

            if 'maskmap' in data_keys:
                batch[i]['maskmap'] = DataContainer(F.pad(
                    batch[i]['maskmap'].data, pad=pad, value=0), stack=batch[i]['maskmap'].stack)

            if 'distance_map' in data_keys:
                batch[i]['distance_map'] = DataContainer(F.pad(
                    batch[i]['distance_map'].data, pad=pad, value=255), stack=batch[i]['distance_map'].stack)

            if 'angle_map' in data_keys:
                batch[i]['angle_map'] = DataContainer(F.pad(
                    batch[i]['angle_map'].data, pad=pad, value=0), stack=batch[i]['angle_map'].stack)

            if 'mask_label_map' in data_keys:
                batch[i]['mask_label_map'] = DataContainer(F.pad(
                    batch[i]['mask_label_map'].data, pad=pad, value=-1), stack=batch[i]['mask_label_map'].stack)

            if 'direction_label_map' in data_keys:
                batch[i]['direction_label_map'] = DataContainer(F.pad(
                    batch[i]['direction_label_map'].data, pad=pad, value=-1), stack=batch[i]['direction_label_map'].stack)

            if 'multi_label_direction_map' in data_keys:
                batch[i]['multi_label_direction_map'] = DataContainer(F.pad(
                    batch[i]['multi_label_direction_map'].data, pad=pad, value=-1), stack=batch[i]['multi_label_direction_map'].stack)

            if 'energy_label_map' in data_keys:
                batch[i]['energy_label_map'] = DataContainer(F.pad(
                    batch[i]['energy_label_map'].data, pad=pad, value=-1), stack=batch[i]['energy_label_map'].stack)

            if 'offsetmap_h' in data_keys:
                batch[i]['offsetmap_h'] = DataContainer(F.pad(
                    batch[i]['offsetmap_h'].data, pad=pad, value=0), stack=batch[i]['offsetmap_h'].stack)

            if 'offsetmap_w' in data_keys:
                batch[i]['offsetmap_w'] = DataContainer(F.pad(
                    batch[i]['offsetmap_w'].data, pad=pad, value=0), stack=batch[i]['offsetmap_w'].stack)
