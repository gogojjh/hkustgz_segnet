...
class ImgWhiteBalancer(object):
    ...
    def __white_balance(self, img):
        '''
        https://blog.csdn.net/qq_36187544/article/details/97657927
    ...
    def process_img_batch(self):
        for img_file in self.__get_img_list(self.input_dir):
            img_name, extension = os.path.splitext(img_file.split('/')[-1])
            img = cv2.imread(img_file)
            balanced_img = self.__white_balance(img)
            
            cv2.imwrite(os.path.join(self.output_dir, '{}.png'.format(img_name)))
