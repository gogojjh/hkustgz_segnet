...
class RampdownScheduler(object):
    ...
    def __init__(self, begin_epoch, max_epoch, current_epoch, max_value, min_value, ramp_mult, configer):
        super().__init__()
        self.begin_epoch = int(begin_epoch)
        self.max_epoch = int(max_epoch)
        self.max_value = float(max_value)
    ...
    @property
    def value(self):
        current_value = self.get_lr(
            self.configer.get('epoch'), self.begin_epoch, self.max_epoch, self.max_value, self.min_value, self.mult)
