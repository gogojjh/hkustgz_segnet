...
class PixelContrastLoss(nn.Module, ABC):
    def __init__(self, configer):
        ...
        self.base_temperature = self.configer.get(
            'contrast', 'base_temperature')
    ...
    def _hard_anchor_sampling(self, X, y_hat, y):
        ...
        for ii in range(batch_size):
            ...
            this_classes = [x for x in this_classes if (
                this_y == x).nonzero().shape[0] > self.max_views]
        ...
        for ii in range(batch_size):
            ...
            for cls_id in this_classes:
                ...
                if num_hard >= n_view / 2 and num_easy >= n_view / 2:
                    ...
                    Log.info('this shoud be never touched! {} {} {}'.format(
    ...
    def _contrastive(self, X_anchor, y_anchor, queue=None):
        ...
        logits_mask = torch.ones_like(mask).scatter_(1,
                                                     torch.arange(
                                                         anchor_num * anchor_count).view(-1, 1).cuda(),
                                                     0)
    ...
    def forward(self, feats, labels=None, predict=None, queue=None):
        ...
        #! labels are less than training features (semi-supervised)
    ...
    def forward(self, preds, target, with_embed=False):
        ...
        if segment_queue is not None and pixel_queue is not None:
            ...
            loss_contrast = self.contrast_criterion(
                embedding, target, predict, queue)
        else:
            loss_contrast = 0
...
class ContrastAuxCELoss(nn.Module, ABC):
    ...
    def forward(self, preds, target):
        ...
        pred = F.interpolate(input=seg, size=(
            h, w), mode='bilinear', align_corners=True)
        pred_aux = F.interpolate(input=seg_aux, size=(
            h, w), mode='bilinear', align_corners=True)
