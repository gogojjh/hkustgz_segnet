...
class Trainer(object):
    ...
    def _init_model(self):
        ...
        if self.with_memory:
            ...
            self.pixel_update_freq = self.configer.get(
                'contrast', 'pixel_update_freq')
    ...
    def _dequeue_and_enqueue(self, keys, labels,
        ...
        for bs in range(batch_size):
            ...
            for lb in this_label_ids:
                ...
                segment_queue[lb, ptr, :] = nn.functional.normalize(
                    feat.view(-1), p=2, dim=0)
                segment_queue_ptr[lb] = (
                    segment_queue_ptr[lb] + 1) % self.memory_size
                ...
                if ptr + K >= self.memory_size:
                    pixel_queue[lb, -K:,
                                :] = nn.functional.normalize(feat, p=2, dim=1)
                    pixel_queue_ptr[lb] = 0
                else:
                    pixel_queue[lb, ptr:ptr + K,
                                :] = nn.functional.normalize(feat, p=2, dim=1)
                    pixel_queue_ptr[lb] = (
                        pixel_queue_ptr[lb] + 1) % self.memory_size
    ...
    def __train(self):
        ...
        for i, data_dict in enumerate(self.train_loader):
            ...
            with_embed = True if self.configer.get(
                'iters') >= self.contrast_warmup_iters else False
            #! use contrastive learning + memory bank
            if self.with_contrast is True:
                if self.with_memory is True:
                    outputs = self.seg_net(
                        *inputs, targets, with_embed=with_embed)
    ...
    def __val(self, data_loader=None):
        ...
        self.module_runner.save_net(
            self.seg_net, save_mode='performance', experiment=None)
        self.module_runner.save_net(
            self.seg_net, save_mode='val_loss', experiment=None)
