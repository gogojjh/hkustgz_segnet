...
# Cityscapes modules
try:
    from metrics.cityscapes.helpers.annotation import Annotation
    from metrics.cityscapes.helpers.labels import labels, name2label, id2label, trainId2label, category2labels
except:
...
def getCsFileInfo(fileName):
    ...
    if not parts:
        printError(
            'Cannot parse given filename ({}). Does not seem to be a valid Cityscapes file.'.format(fileName))
    if len(parts) == 5:
        csFile = CsFile(*parts[:-1], type2="", ext=parts[-1])
    elif len(parts) == 6:
        csFile = CsFile(*parts)
    else:
        printError('Found {} part(s) in given filename ({}). Expected 5 or 6.'.format(
            len(parts), fileName))
...
def writeDict2JSON(dictName, fileName):
    with open(fileName, 'w') as f:
        f.write(json.dumps(dictName, default=lambda o: o.__dict__,
