...
from extensions.pacnet import pac
...
def create_YXRGB(img, yx_scale=None, rgb_scale=None, scales=None):
    ...
    if scales is not None:
        ...
        yx_scale = scales[:2]
        rgb_scale = scales[2:]
    mesh = create_position_feats(
        img.shape[-2:], yx_scale, img.shape[0], img.device)
    if rgb_scale is not None:
        if not isinstance(rgb_scale, th.Tensor):
            rgb_scale = th.tensor(
                rgb_scale, dtype=th.float32, device=img.device)
...
class PacCRF(nn.Module):
    ...
    def __init__(self, channels, num_steps, final_output='log_Q', perturbed_init=True, native_impl=False,
        ...
        self.unary_weight = unary_weight if self.fixed_weighting else nn.Parameter(
            th.tensor(float(unary_weight)))
        self.blur = []
        self.pairwise_repr = []
    ...
    def reset_parameters(self, pairwise_idx=None):
        ...
        for i in idxs:
            ...
            if self.compat[i] is not None:
                self.compat[i].weight.data[:, :, 0, 0] = 1.0 - \
                    th.eye(self.channels, dtype=th.float32)
                if self.perturbed_init:
                    perturb_range = 0.001
                    self.compat[i].weight.data.add_(
                        (th.rand_like(self.compat[i].weight.data) - 0.5) * perturb_range)
            self.pairwise_weights[i].data = th.ones_like(
                self.pairwise_weights[i]) * self.init_pairwise_weights[i]
    ...
    def extra_repr(self):
        ...
        if self.pairwise_repr:
            s += ', pairwise_kernels=({})'.format(
                ', '.join(self.pairwise_repr))
    ...
    def add_pairwise_kernel(self, kernel_size=3, dilation=1, blur=1, compat_type='4d', spatial_filter=True,
        ...
        self.pairwise_weights.append(nn.Parameter(
            th.tensor(pairwise_weight, dtype=th.float32)))
        ...
        if compat_type == 'na':
            messenger = nn.Conv2d(self.channels, self.channels,
    ...
    def forward(self, unary, edge_feat, edge_kernel=None, logQ=None):
        ...
        edge_kernel = [
            edge_kernel] * n_kernels if isinstance(edge_kernel, th.Tensor) else edge_kernel
        ...
        for step in range(self.num_steps):
            ...
            for i in range(n_kernels):
                ...
                if isinstance(self.messengers[i], nn.Conv2d):
                    ...
                    msg = self.messengers[i](
                        Q_blur[self.blur[i]], None, edge_kernel[i])
                if self.compat[i] is not None:
                    msg = self.compat[i](msg)
                if self.blur[i] > 1:
                    msg = F.interpolate(
                        msg, scale_factor=self.blur[i], mode='bilinear', align_corners=False)
                    msg = msg[:, :, pad[0]:pad[0] + unary.shape[2],
                              pad[1]:pad[1] + unary.shape[3]].contiguous()
        ...
        if self.final_output == 'softmax':
            ...
            raise ValueError(
                'Unknown value for final_output: {}'.format(self.final_output))
...
class PacCRFLoose(nn.Module):
    ...
    def add_pairwise_kernel(self, kernel_size=3, dilation=1, blur=1, compat_type='4d', spatial_filter=True,
                            pairwise_weight=1.0):
        for i in range(self.num_steps):
            self.steps[i].add_pairwise_kernel(
                kernel_size, dilation, blur, compat_type, spatial_filter, pairwise_weight)
